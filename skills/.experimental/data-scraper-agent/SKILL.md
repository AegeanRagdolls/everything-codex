...
description: >-
  Build a fully automated AI-powered data collection agent for any public source — job
  boards, prices, news, GitHub, sports, anything. Scrapes on a schedule, enriches data
  with a free LLM (Gemini Flash), stores results in Notion/Sheets/Supabase, and learns
  from user feedback. Runs 100% free on GitHub Actions. Use when the user wants to
  monitor, collect, or track any public data automatically. Do not use for unrelated
  tasks. Inputs should include relevant files, constraints, and available tools. Output
  should be a concise plan, result, or verification summary.
---
