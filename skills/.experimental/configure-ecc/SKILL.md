...
name: configure-ecc
description: >-
  Interactive installer for Everything Claude Code — guides users through selecting and
  installing skills and rules to user-level or project-level directories, verifies
  paths, and optionally optimizes installed files. Use when Codex needs support for:
  Interactive installer for Everything Claude Code — guides users through selecting and
  installing skills and rules to user-level or project-level directories, verifies
  paths, and optionally optimizes installed files. Do not use for unrelated tasks.
...
# Configure Everything Codex Code (ECC)

An interactive, step-by-step installation wizard for the Everything Codex Code project. Uses `AskUserQuestion` to guide users through selective installation of skills and rules, then verifies correctness and offers optimization.
...
1. **Via Plugin**: `/plugin install everything-codex-code` — the plugin loads this skill automatically
...
```bash
rm -rf /tmp/everything-codex-code
git clone https://github.com/affaan-m/everything-codex-code.git /tmp/everything-codex-code
```

Set `ECC_ROOT=/tmp/everything-codex-code` as the source for all subsequent copy operations.
