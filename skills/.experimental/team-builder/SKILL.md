...
name: team-builder
description: >-
  Interactive agent picker for composing and dispatching parallel teams. Use when Codex
  needs support for: Interactive agent picker for composing and dispatching parallel
  teams. Do not use for unrelated tasks. Inputs should include relevant files,
  constraints, and available tools. Output should be a concise plan, result, or
...
1. **`claude agents` command** (primary) — run `claude agents` to get all agents known to the CLI, including user agents, plugin agents (e.g. `everything-codex-code:architect`), and built-in agents. This automatically covers ECC marketplace installs without any path configuration.
...
- **Plugin agents** are prefixed with `plugin-name:` (e.g., `everything-codex-code:security-reviewer`). Use the part after `:` as the agent name and the plugin name as the domain.
