...
description: >-
  Use when auditing Claude skills and commands for quality. Supports Quick Scan (changed
  skills only) and Full Stocktake modes with sequential subagent batch evaluation. Do
...
Codex prompt recipe equivalent: ask Codex to execute the `skill-stocktake` workflow as normal prompt steps.
```
