# Prp Implement Prompt Recipe

## Codex invocation

Ask Codex to execute the `prp-implement` workflow as a normal prompt. Provide the target files, repository context, constraints, and whether edits are allowed.
...
This is a Codex prompt recipe, not a Claude slash command. In Codex, paste this file's body into the chat or ask: "Use the `prp-implement` prompt recipe for <task>."

**Original command intent:** Execute an implementation plan with rigorous validation loops. Use when Codex needs support for: Execute an implementation plan with rigorous validation loops.
...
```bash
mkdir -p .claude/PRPsthe plan prompt recipes/completed
mv "$ARGUMENTS" .claude/PRPsthe plan prompt recipes/completed/
```
...
- Archived Plan: `.claude/PRPsthe plan prompt recipes/completed/{name}.plan.md`
...
> Next step: Use the Codex prompt recipe equivalent for `prp-pr` to create a pull request, or `/code-review` to review changes first.
```
...
- Use the Codex prompt recipe equivalent for `code-review` to review changes before committing
- Use the Codex prompt recipe equivalent for `prp-commit` to commit with a descriptive message
- Use the Codex prompt recipe equivalent for `prp-pr` to create a pull request
- Use the Codex prompt recipe equivalent for `prp-plan` <next-phase> if the PRD has more phases
