# Python Review Prompt Recipe

## Codex invocation

Ask Codex to execute the `python-review` workflow as a normal prompt. Provide the target files, repository context, constraints, and whether edits are allowed.
...
This is a Codex prompt recipe, not a Claude slash command. In Codex, paste this file's body into the chat or ask: "Use the `python-review` prompt recipe for <task>."

**Original command intent:** Comprehensive Python code review for PEP 8 compliance, type hints, security, and Pythonic idioms. Invokes the python-reviewer agent. Use when Codex needs support for: Comprehensive Python code review for PEP 8 compliance, type hints, security, and Pythonic idioms. Invokes the python-reviewer agent.
...
use the Codex prompt recipe equivalent for `python-review` when:
...
- use the Codex prompt recipe equivalent for `code-review` for non-Python specific concerns
- use the Codex prompt recipe equivalent for `python-review` before committing
- use the Codex prompt recipe equivalent for `build-fix` if static analysis tools fail
