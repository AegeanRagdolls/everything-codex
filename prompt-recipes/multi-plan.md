# Multi Plan Prompt Recipe

## Codex invocation

Ask Codex to execute the `multi-plan` workflow as a normal prompt. Provide the target files, repository context, constraints, and whether edits are allowed.
...
This is a Codex prompt recipe, not a Claude slash command. In Codex, paste this file's body into the chat or ask: "Use the `multi-plan` prompt recipe for <task>."

**Original command intent:** multi plan. Use when Codex needs support for: multi plan.
...
- **Planning Only**: This command allows reading context and writing to `.claudethe plan prompt recipe/*` plan files, but **NEVER modify production code**
...
2. Save plan to `.claudethe plan prompt recipe/<feature-name>.md` (extract feature name from requirement, e.g., `user-auth`, `payment-module`)
3. Output prompt in **bold text** (MUST use actual saved file path):

---
**Plan generated and saved to `.claudethe plan prompt recipe/actual-feature-name.md`**
...
```
Codex prompt recipe equivalent: ask Codex to execute the `ccg:execute` workflow with arguments `.claudethe plan prompt recipe/actual-feature-name.md`.
```
---
...
- **First planning**: `.claudethe plan prompt recipe/<feature-name>.md`
- **Iteration versions**: `.claudethe plan prompt recipe/<feature-name>-v2.md`, `.claudethe plan prompt recipe/<feature-name>-v3.md`...
...
2. Update `.claudethe plan prompt recipe/<feature-name>.md` file
...
```bash
Codex prompt recipe equivalent: ask Codex to execute the `ccg:execute` workflow with arguments `.claudethe plan prompt recipe/<feature-name>.md`.
```
