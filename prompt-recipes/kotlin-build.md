# Kotlin Build Prompt Recipe

## Codex invocation

Ask Codex to execute the `kotlin-build` workflow as a normal prompt. Provide the target files, repository context, constraints, and whether edits are allowed.
...
This is a Codex prompt recipe, not a Claude slash command. In Codex, paste this file's body into the chat or ask: "Use the `kotlin-build` prompt recipe for <task>."

**Original command intent:** Fix Kotlin/Gradle build errors, compiler warnings, and dependency issues incrementally. Invokes the kotlin-build-resolver agent for minimal, surgical fixes. Use when Codex needs support for: Fix Kotlin/Gradle build errors, compiler warnings, and dependency issues incrementally. Invokes the kotlin-build-resolver agent for minimal, surgical fixes.
...
use the Codex prompt recipe equivalent for `kotlin-build` when:
