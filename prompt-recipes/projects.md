# Projects Prompt Recipe

## Codex invocation

Ask Codex to execute the `projects` workflow as a normal prompt. Provide the target files, repository context, constraints, and whether edits are allowed.
...
This is a Codex prompt recipe, not a Claude slash command. In Codex, paste this file's body into the chat or ask: "Use the `projects` prompt recipe for <task>."

**Original command intent:** List known projects and their instinct statistics. Use when Codex needs support for: List known projects and their instinct statistics.
...
```bash
Codex prompt recipe equivalent: ask Codex to execute the `projects` workflow.
```
