# Multi Execute Prompt Recipe

## Codex invocation

Ask Codex to execute the `multi-execute` workflow as a normal prompt. Provide the target files, repository context, constraints, and whether edits are allowed.
...
This is a Codex prompt recipe, not a Claude slash command. In Codex, paste this file's body into the chat or ask: "Use the `multi-execute` prompt recipe for <task>."

**Original command intent:** multi execute. Use when Codex needs support for: multi execute.
...
1. **Identify Input Type**:
   - Plan file path (e.g., `.claudethe plan prompt recipe/xxx.md`)
...
Codex prompt recipe equivalent: ask Codex to execute the `ccg:execute` workflow with arguments `implement user authentication based on previous plan`.
```
