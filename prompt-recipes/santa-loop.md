# Santa Loop Prompt Recipe

## Codex invocation

Ask Codex to execute the `santa-loop` workflow as a normal prompt. Provide the target files, repository context, constraints, and whether edits are allowed.
...
This is a Codex prompt recipe, not a Claude slash command. In Codex, paste this file's body into the chat or ask: "Use the `santa-loop` prompt recipe for <task>."

**Original command intent:** Adversarial dual-review convergence loop — two independent model reviewers must both approve before code ships. Use when Codex needs support for: Adversarial dual-review convergence loop — two independent model reviewers must both approve before code ships.
...
```
Codex prompt recipe equivalent: ask Codex to execute the `santa-loop` workflow with arguments `[file-or-glob | description]`.
```
