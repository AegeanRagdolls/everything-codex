# Loop Start Prompt Recipe

## Codex invocation

Ask Codex to execute the `loop-start` workflow as a normal prompt. Provide the target files, repository context, constraints, and whether edits are allowed.
...
This is a Codex prompt recipe, not a Claude slash command. In Codex, paste this file's body into the chat or ask: "Use the `loop-start` prompt recipe for <task>."

**Original command intent:** loop start. Use when Codex needs support for: loop start.
...
4. Create loop plan and write runbook under `.claudethe plan prompt recipes/`.
