...
Ask Codex to execute the `tdd` workflow as a normal prompt. Provide the target files, repository context, constraints, and whether edits are allowed.
...
This is a Codex prompt recipe, not a Claude slash command. In Codex, paste this file's body into the chat or ask: "Use the `tdd` prompt recipe for <task>."

**Original command intent:** Legacy slash-entry shim for the tdd-workflow skill. Prefer the skill directly. Use when Codex needs support for: Legacy slash-entry shim for the tdd-workflow skill. Prefer the skill directly.

---

# TDD Command (Legacy Shim)

Use this only if you still invoke `the tdd-workflow skill`. The maintained workflow lives in `skillsthe tdd-workflow skill-workflow/SKILL.md`.
...
**E2E Tests** (use the Codex prompt recipe equivalent for `e2e` command):
- Critical user flows
...
- Use `the plan prompt recipe` first to understand what to build
- Use `the tdd-workflow skill` to implement with tests
- use the Codex prompt recipe equivalent for `build-fix` if build errors occur
- use the Codex prompt recipe equivalent for `code-review` to review implementation
- use the Codex prompt recipe equivalent for `test-coverage` to verify coverage
...
- `agentsthe tdd-workflow skill-guide.md`
- `skillsthe tdd-workflow skill-workflow/SKILL.md`
