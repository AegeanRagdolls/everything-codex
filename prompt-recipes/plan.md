...
Ask Codex to execute the `plan` workflow as a normal prompt. Provide the target files, repository context, constraints, and whether edits are allowed.
...
This is a Codex prompt recipe, not a Claude slash command. In Codex, paste this file's body into the chat or ask: "Use the `plan` prompt recipe for <task>."

**Original command intent:** Restate requirements, assess risks, and create step-by-step implementation plan. WAIT for user CONFIRM before touching any code. Use when Codex needs support for: Restate requirements, assess risks, and create step-by-step implementation plan. WAIT for user CONFIRM before touching any code.
...
Use `the plan prompt recipe` when:
...
```
User: the plan prompt recipe I need to add real-time notifications when markets resolve
...
- use the Codex prompt recipe equivalent for `code-review` to review completed implementation

> **Need deeper planning?** use the Codex prompt recipe equivalent for `prp-plan` for artifact-producing planning with PRD integration, codebase analysis, and pattern extraction. use the Codex prompt recipe equivalent for `prp-implement` to execute those plans with rigorous validation loops.
...
`agentsthe plan prompt recipener.md`
