...
Ask Codex to execute the `prune` workflow as a normal prompt. Provide the target files, repository context, constraints, and whether edits are allowed.
...
This is a Codex prompt recipe, not a Claude slash command. In Codex, paste this file's body into the chat or ask: "Use the `prune` prompt recipe for <task>."

**Original command intent:** Delete pending instincts older than 30 days that were never promoted. Use when Codex needs support for: Delete pending instincts older than 30 days that were never promoted.
...
```
Codex prompt recipe equivalent: ask Codex to execute the `prune` workflow with arguments `# Delete instincts older than 30 days`.
Codex prompt recipe equivalent: ask Codex to execute the `prune` workflow with arguments `--max-age 60      # Custom age threshold (days)`.
Codex prompt recipe equivalent: ask Codex to execute the `prune` workflow with arguments `--dry-run         # Preview without deleting`.
```
