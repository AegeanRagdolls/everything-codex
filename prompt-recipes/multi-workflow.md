# Multi Workflow Prompt Recipe

## Codex invocation

Ask Codex to execute the `multi-workflow` workflow as a normal prompt. Provide the target files, repository context, constraints, and whether edits are allowed.
...
This is a Codex prompt recipe, not a Claude slash command. In Codex, paste this file's body into the chat or ask: "Use the `multi-workflow` prompt recipe for <task>."

**Original command intent:** multi workflow. Use when Codex needs support for: multi workflow.
...
```bash
Codex prompt recipe equivalent: ask Codex to execute the `workflow` workflow with arguments `<task description>`.
```
...
```bash
node scripts/orchestrate-worktrees.js .claudethe plan prompt recipe/workflow-e2e-test.json --execute
```
...
**Claude Synthesis**: Adopt Codex backend plan + Gemini frontend plan, save to `.claudethe plan prompt recipe/task-name.md` after user approval.
