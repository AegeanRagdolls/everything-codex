# Prp Plan Prompt Recipe

## Codex invocation

Ask Codex to execute the `prp-plan` workflow as a normal prompt. Provide the target files, repository context, constraints, and whether edits are allowed.
...
This is a Codex prompt recipe, not a Claude slash command. In Codex, paste this file's body into the chat or ask: "Use the `prp-plan` prompt recipe for <task>."

**Original command intent:** Create comprehensive feature implementation plan with codebase analysis and pattern extraction. Use when Codex needs support for: Create comprehensive feature implementation plan with codebase analysis and pattern extraction.
...
Write the full plan document using the template below. Save to `.claude/PRPsthe plan prompt recipes/{kebab-case-feature-name}.plan.md`.

Create the directory if it doesn't exist:
```bash
mkdir -p .claude/PRPsthe plan prompt recipes
```
...
```
.claude/PRPsthe plan prompt recipes/{kebab-case-feature-name}.plan.md
```
...
- **File**: .claude/PRPsthe plan prompt recipes/{kebab-case-feature-name}.plan.md
- **Source PRD**: [path or "N/A"]
...
> Next step: Use the Codex prompt recipe equivalent for `prp-implement` .claude/PRPsthe plan prompt recipes/{name}.plan.md to execute this plan.
```
...
- Use the Codex prompt recipe equivalent for `prp-implement` <plan-path> to execute this plan
- Run `the plan prompt recipe` for quick conversational planning without artifacts
- Use the Codex prompt recipe equivalent for `prp-prd` to create a PRD first if scope is unclear
````
