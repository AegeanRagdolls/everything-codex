# Prp Prd Prompt Recipe

## Codex invocation

Ask Codex to execute the `prp-prd` workflow as a normal prompt. Provide the target files, repository context, constraints, and whether edits are allowed.
...
This is a Codex prompt recipe, not a Claude slash command. In Codex, paste this file's body into the chat or ask: "Use the `prp-prd` prompt recipe for <task>."

**Original command intent:** Interactive PRD generator - problem-first, hypothesis-driven product spec with back-and-forth questioning. Use when Codex needs support for: Interactive PRD generator - problem-first, hypothesis-driven product spec with back-and-forth questioning.
...
- use the Codex prompt recipe equivalent for `prp-plan` to create implementation plans from PRD phases
- Use `the plan prompt recipe` for simpler planning without PRD structure
- use the Codex prompt recipe equivalent for `save-session` to preserve PRD context across sessions
