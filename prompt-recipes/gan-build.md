# GAN Build Prompt Recipe

## Codex invocation

Ask Codex to execute the `gan-build` workflow as a normal prompt. Provide the target files, repository context, constraints, and whether edits are allowed.
...
This is a Codex prompt recipe, not a Claude slash command. In Codex, paste this file's body into the chat or ask: "Use the `gan-build` prompt recipe for <task>."

**Original command intent:** gan build. Use when Codex needs support for: gan build.
