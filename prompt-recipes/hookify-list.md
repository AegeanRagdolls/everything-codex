# Hookify List Prompt Recipe

## Codex invocation

Ask Codex to execute the `hookify-list` workflow as a normal prompt. Provide the target files, repository context, constraints, and whether edits are allowed.
...
This is a Codex prompt recipe, not a Claude slash command. In Codex, paste this file's body into the chat or ask: "Use the `hookify-list` prompt recipe for <task>."

**Original command intent:** List all configured hookify rules. Use when Codex needs support for: List all configured hookify rules.
