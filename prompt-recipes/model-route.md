# Model Route Prompt Recipe

## Codex invocation

Ask Codex to execute the `model-route` workflow as a normal prompt. Provide the target files, repository context, constraints, and whether edits are allowed.
...
This is a Codex prompt recipe, not a Claude slash command. In Codex, paste this file's body into the chat or ask: "Use the `model-route` prompt recipe for <task>."

**Original command intent:** model route. Use when Codex needs support for: model route.
