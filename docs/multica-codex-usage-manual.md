...
如果你只想看脚本参数，读 [docs/multica-codex-team-bootstrap.md](multica-codex-team-bootstrap.md)。如果你想知道这套东西到底怎么工作、怎么排障、怎么日常使用，读这份手册。
...
- `everything-codex-code`：给 Codex 提供 skills、agents、prompt recipes 和项目规则的能力包。推荐来源是 `https://github.com/AegeanRagdolls/everything-codex`，本地 clone 到哪里都可以。

注意：`everything-codex-code` 是能力包，不是所有新需求的默认业务代码仓库。用户提出模糊新产品想法时，PM 应先澄清目标用户、平台、范围、验收标准和目标项目路径；在用户明确确认进入执行前，不应创建 Dev/Test/Review 子任务。没有目标项目路径或 Gitea 仓库时，PM 只能读取 Multica issue/comment 做产品澄清或 PRD 收敛，不应运行 `pwd`、`rg --files`、`git status` 这类仓库探测命令。

实际工作流是：

```text
你在 Multica 里提需求
-> 产品经理 / 项目调度 负责澄清、拆任务、分配
...
如果你只是要把能力包接入 Multica，不绑定任何业务项目路径，可以直接运行：
...
如果你要把 **某个现有业务项目** 接入 Multica，再把目标项目路径作为最后一个参数传入。`--create-smoke-issue` 也必须显式传目标路径，避免把能力包目录误当成业务项目。

## 一键脚本会做什么

[scripts/bootstrap-multica-codex-team.sh](../scripts/bootstrap-multica-codex-team.sh) 默认会做这些事：
...
6. 如果提供了目标项目路径，把它作为“可选绑定项目路径”写进 Agent instructions；未提供时显示“未绑定”
...
- 用户追加复杂产品设想时先进入追问/需求再收敛，更新 PRD/MVP/路线图，不自动派发开发
- 只有用户明确确认范围并要求开始拆任务或开始开发后，PM 才能分配 Dev / Tester / Reviewer
- `everything-codex-code` 只作为 Codex 能力包，不能被自动推断为目标产品仓库
- Dev / Tester / Reviewer 必须从 issue 中读取目标项目路径或 Gitea 仓库；缺失时回报 blocker
...
- 启动脚本：[scripts/start-multica-daemon-wsl.ps1](../scripts/start-multica-daemon-wsl.ps1)
- WSL 内自检脚本：[scripts/ensure-multica-daemon.sh](../scripts/ensure-multica-daemon.sh)
...
最直接的方式是在 Multica 面板里和 `产品经理 / 项目调度` 沟通。
...
- [docs/multica-codex-team-bootstrap.md](multica-codex-team-bootstrap.md)：bootstrap 参数和对象说明
- [scripts/bootstrap-multica-codex-team.sh](../scripts/bootstrap-multica-codex-team.sh)：一键接入脚本
- [scripts/ensure-multica-daemon.sh](../scripts/ensure-multica-daemon.sh)：daemon 自检启动脚本
- [scripts/install-windows-wsl-multica-autostart.sh](../scripts/install-windows-wsl-multica-autostart.sh)：Windows 登录自启动安装器
